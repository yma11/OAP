...
private[filecache] class CacheMemoryAllocator(sparkEnv: SparkEnv)
  ...
  private def init(): (MemoryManager, MemoryManager) = {
    if (_separateMemory) {
      val dataManager = MemoryManager(sparkEnv, OapConf.OAP_FIBERCACHE_STRATEGY, "data")
      val indexManager = MemoryManager(sparkEnv, OapConf.OAP_FIBERCACHE_STRATEGY, "index")
